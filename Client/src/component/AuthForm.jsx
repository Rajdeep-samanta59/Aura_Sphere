...
const BASE_URL = "https://aura-sphere.vercel.app/api/auth"; // Corrected base URL with /api/auth
...
export default function AuthForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = mode === "signup" ? `${BASE_URL}/register` : `${BASE_URL}/login`;
      ...
      if (mode === "signup") {
        ...
        if (avatar) {
          ...
          console.warn("No avatar provided; backend may reject if avatar is required");
        }
  ...
  const handleSocialAuth = (provider) => {
    const url = provider === "google" ? `${BASE_URL}/google` : `${BASE_URL}/github`;
    window.location.href = url;
  };
